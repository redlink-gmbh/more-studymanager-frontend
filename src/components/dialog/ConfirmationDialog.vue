...
<script setup lang="ts">
  ...
  const message: string =
    dialogRef.value?.data?.message ?? t('global.labels.leavePage');
  const cancelBtn: string =
    dialogRef.value?.data?.cancelBtn ?? t('global.labels.cancel');
  const approveBtn: string =
    dialogRef.value?.data?.approveBtn ?? t('global.labels.confirm');
